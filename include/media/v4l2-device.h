...
/**
 * v4l2_device_register - 初始化 v4l2_dev，并让 @dev->driver_data 
 *                        指向 @v4l2_dev。
 *
 * @dev: 指向 struct &device 的指针（代表底层的物理设备）
 * @v4l2_dev: 指向 struct &v4l2_device 的指针（V4L2 的设备句柄）
 *
 * .. note:: (注意：)
 *    在极少数情况下（比如 ISA 设备），@dev 可能是 %NULL（空指针）。
 *    在这种情况下，调用者必须在调用此函数之前，
 *    手动填充好 @v4l2_dev->name 字段。
