 ...
 */
 /**
 * struct vb2_queue - videobuf 队列结构体
 *
 * @type: 私有缓冲区类型，其内容由 vb2-core 调用者定义。
 *        例如，对于 V4L2，它应该匹配 &enum v4l2_buf_type 中定义的类型。
 * @io_modes: 支持的 IO 方法（参见 &enum vb2_io_modes）
 * @alloc_devs: 内存类型/分配器特定的每个平面的设备结构体
 * @dev: 如果驱动没有填充 @alloc_devs 数组，则用于默认分配上下文的设备
 * @dma_attrs: 用于 DMA 的 DMA 属性
 * @bidirectional: 当设置此标志时，此队列缓冲区的 DMA 方向将被 %DMA_BIDIRECTIONAL 覆盖。
 *                 这在硬件（固件）写入映射为读取（%DMA_TO_DEVICE）的缓冲区，
 *                 或从映射为写入（%DMA_FROM_DEVICE）的缓冲区读取时很有用，
 *                 以满足某些内部硬件限制或添加处理算法所需的填充。
 *                 如果 DMA 映射不是双向的，但硬件（固件）试图以相反方向访问缓冲区，
 *                 可能会导致 IOMMU 保护故障。
 * @fileio_read_once: 读取第一个缓冲区后报告 EOF
 * @fileio_write_immediately: 每次 write() 调用后立即将缓冲区入队
 * @allow_zero_bytesused: 允许将 bytesused == 0 传递给驱动
 * @quirk_poll_must_check_waiting_for_buffers: 当尚未调用 QBUF 时，在 poll 时返回 %EPOLLERR。
 *         这是一个 vb1 的习惯用法，也被 vb2 采用。
 * @supports_requests: 此队列支持 Request API
 * @requires_requests: 此队列需要 Request API。如果设置为 1，
 *                     则 supports_requests 也必须设置为 1
 * @uses_qbuf: qbuf 被直接用于此队列。首次调用时设置为 1，
 *             队列取消时设置为 0。如果此为 1，则不能从 request 中入队缓冲区。
 * @uses_requests: request 被用于此队列。首次入队 request 时设置为 1，
 *                 队列取消时设置为 0。如果此为 1，则不能直接入队缓冲区。
 * @allow_cache_hints: 当设置时，用户空间可以传递缓存管理提示，
 *                     以跳过 ->prepare() 和/或 ->finish() 上的缓存刷新/无效化操作
 * @lock: 保护 &struct vb2_queue 的互斥锁指针。
 *        驱动可以将其设置为互斥锁，让 v4l2 核心序列化队列 ioctl。
 *        如果驱动想要自己处理锁定，则应将此设置为 NULL。
 *        此锁不被 videobuf2 核心 API 使用。
 * @owner: 拥有缓冲区的文件句柄，即调用 reqbufs、create_buffers 或启动 fileio 的文件句柄。
 *         此字段不被 videobuf2 核心 API 使用，但它允许驱动轻松地将所有者文件句柄与队列关联。
 * @ops: 驱动特定的回调函数
 * @mem_ops: 内存分配器特定的回调函数
 * @buf_ops: 传递缓冲区信息的回调函数，用于用户空间和内核空间之间
 * @drv_priv: 驱动私有数据
 * @subsystem_flags: 子系统特定的标志（V4L2/DVB/等）。不被 vb2 核心使用。
 * @buf_struct_size: 驱动特定缓冲区结构体的大小；
 *                   "0" 表示驱动不想使用自定义缓冲区结构体类型。
 *                   在这种情况下，将使用子系统特定的结构体（对于 V4L2 来说是
 *                   ``sizeof(struct vb2_v4l2_buffer)``）。
 *                   驱动特定缓冲区结构体的第一个字段必须是子系统特定的结构体
 *                   （对于 V4L2 来说是 vb2_v4l2_buffer）。
 * @timestamp_flags: 时间戳标志；``V4L2_BUF_FLAG_TIMESTAMP_*`` 和
 *                   ``V4L2_BUF_FLAG_TSTAMP_SRC_*``
 * @gfp_flags: 分配缓冲区时使用的额外 gfp 标志。
 *             通常为 0，但可以是例如 %GFP_DMA 或 %__GFP_DMA32
 *             以强制将缓冲区分配到特定的内存区域。
 * @min_buffers_needed: 在可以调用 @start_streaming 之前所需的最小缓冲区数量。
 *                      当 DMA 引擎需要至少有一定数量的缓冲区入队才能启动时使用。
 */

/*
 * 私有元素（不会出现在 uAPI 文档中）：
 * @mmap_lock: 分配/释放/内存映射缓冲区时使用的私有互斥锁
 * @memory: 当前使用的内存类型
 * @dma_dir: DMA 映射方向
 * @bufs: videobuf 缓冲区结构体数组
 * @num_buffers: 已分配/使用的缓冲区数量
 * @queued_list: 当前从用户空间入队的缓冲区列表
 * @queued_count: 已入队并准备好进行流式传输的缓冲区数量
 * @owned_by_drv_count: 驱动拥有的缓冲区数量
 * @done_list: 准备好被用户空间出队的缓冲区列表
 * @done_lock: 保护 done_list 链表的锁
 * @done_wq: 等待缓冲区准备好被出队的进程的等待队列
 * @streaming: 当前流式传输状态
 * @start_streaming_called: @start_streaming 已被成功调用并且我们已开始流式传输
 * @error: 队列上发生了致命错误
 * @waiting_for_buffers: 在 poll() 中用于检查 vb2 是否仍在等待缓冲区。
 *                       仅对采集队列设置，如果尚未调用 qbuf，因为 poll() 需要在这种情况下返回 %EPOLLERR
 * @is_multiplanar: 如果缓冲区类型是多平面则设置
 * @is_output: 如果缓冲区类型是输出则设置
 * @copy_timestamp: 如果 vb2-core 应设置时间戳则设置
 * @last_buffer_dequeued: 在 poll() 和 DQBUF 中使用，如果最后一个解码的缓冲区已被出队则立即返回。
 *                        当带有 %V4L2_BUF_FLAG_LAST 标志的缓冲区被出队时，为采集队列设置
 * @fileio: 文件 IO 模拟器内部数据，仅在模拟器激活时使用
 * @threadio: 线程 IO 内部数据，仅在线程激活时使用
 * @name: 队列名称，用于日志记录目的。如果驱动留空则自动初始化
 */
struct vb2_queue {
	unsigned int			type;
