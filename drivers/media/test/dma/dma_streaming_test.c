...
static int dma_test_probe(struct platform_device *pdev)
{
	 ...
	 */
	tdev->src_cpu_addr = kmalloc(DMA_SIZE, GFP_KERNEL);
	 ...
	 */
	tdev->dst_cpu_addr = kmalloc(DMA_SIZE, GFP_KERNEL);
	 ...
	 */
	memset(tdev->src_cpu_addr, 0xAA, DMA_SIZE);
	tdev->src_dma_addr = dma_map_single(tdev->dev, tdev->src_cpu_addr, DMA_SIZE,DMA_TO_DEVICE);
	memset(tdev->src_cpu_addr, 0xBB, DMA_SIZE);
	dma_sync_single_for_device(tdev->dev, tdev->src_dma_addr, DMA_SIZE, DMA_TO_DEVICE);
	tdev->dst_dma_addr = dma_map_single(tdev->dev, tdev->dst_cpu_addr, DMA_SIZE, DMA_FROM_DEVICE);
	...
	dma_unmap_single(tdev->dev, tdev->src_dma_addr, DMA_SIZE, DMA_TO_DEVICE);
	dma_unmap_single(tdev->dev, tdev->dst_dma_addr, DMA_SIZE, DMA_FROM_DEVICE);
	/*
	 * 搬运后打印前 100 字节。
	 *
	 * 你应该能在 dmesg 里看到：
