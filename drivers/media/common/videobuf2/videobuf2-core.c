...
/*
 * vb2_start_streaming() - 尝试开始流传输。
 * @q:		videobuf2 队列
 *
 * 尝试开始流传输。当调用这个函数时，必须至少有 q->min_buffers_needed 
 * 个缓冲区已经入队（即 DMA 引擎正常工作所需的最小缓冲区数量）。
 * 如果 start_streaming 操作失败，它应该将所有驱动拥有的缓冲区以 QUEUED 
 * 状态返回给 vb2 框架。检查这种情况是否发生，如果没有发生，则发出警告
 * 并强制回收它们。
 */
/*
...
static int vb2_start_streaming(struct vb2_queue *q)
{
	...
	/*
	* 如果在 streamon 之前有任何缓冲区已经入队，
	* 那么我们现在可以将它们传递给驱动程序进行处理。
	*/
	/*
		  ...
		  struct media_request *req)
{
	...
	if (req) {
		...
		vb->state = VB2_BUF_STATE_IN_REQUEST;
			/*
			* 增加引用计数并存储请求（request）。
			* 当缓冲区出队（dequeued）时，请求的引用计数会再次减少。
			* 这样做是为了防止 vb2_buffer_done() 在中断上下文中释放请求（request），
			* 这种情况可能发生在应用程序在将请求排队（queue）之后关闭了请求的文件描述符（fd）。
			*/
		/*
	...
	/*
	* 将缓冲区添加到已排队（queued）缓冲区链表中。
	* 一个缓冲区会一直保留在这个链表中，直到在 dqbuf（出队操作）中被移除。
	*/
	...
	trace_vb2_qbuf(q, vb);
	/*
	* 如果已经在 streaming（流传输）状态，则将缓冲区交给驱动处理。
	* 如果不是，则该缓冲区将在下一次 streamon（开启流传输）时交给驱动。
 	*/
	/*
	...
	/*
	* 如果 streamon 已经被调用，
	* 并且由于之前排队的缓冲区数量不足，我们还没有调用 start_streaming()，
	* 而现在我们终于达到了所需的最小缓冲区数量，
	* 那么我们就可以调用 start_streaming() 了。
	*/
