...
/* 视频负载（payload）的解码由 uvc_video_decode_start(),
 * uvc_video_decode_data() 和 uvc_video_decode_end() 处理。
 *
 * uvc_video_decode_start 在 bulk 或 isochronous 传输的 URB 数据开始时被调用。
 * 它处理头部数据，成功后返回头部大小（字节数）。如果出错，返回负错误码。
 * 以下错误码有特殊含义：
 *
 * - EAGAIN：告知调用者：当前的视频缓冲区应该被标记为“完成”，
 *   然后应该用同样的数据和一个新的视频缓冲区再次调用本函数。
 *   这个机制用于“只在下一帧开始时才能可靠检测到当前帧结束”的情况。
 *
 * 如果返回的是 EAGAIN 以外的错误，调用者会丢弃当前负载。
 * 在下一次负载到来之前，不会调用 uvc_video_decode_data 和 uvc_video_decode_end。
 * 如果没有合适的错误码，可以用 -ENODATA 来丢弃当前负载。
 *
 * uvc_video_decode_data 会在每个包含负载数据的 URB 上被调用。
 * 它将数据拷贝到视频缓冲区。
 *
 * uvc_video_decode_end 在 bulk 或 isochronous 负载的末尾被调用，
 * 并传入头部数据。它执行额外的头部数据处理，成功返回 0，失败返回负错误码。
 * 由于头部数据已经在 uvc_video_decode_start 中处理过，本函数不需要再做第二次完整性检查。
 *
 * 对于等时传输（isochronous），一个负载总是通过单个 URB 传输，
 * 因此这三个函数会连续被调用。
 *
 * 为了让解码器即使在“没有可用视频缓冲区”时也能处理头部数据并更新内部状态，
 * uvc_video_decode_start 必须能接受 buf 参数为 NULL 的情况。
 * 而 uvc_video_decode_data 和 uvc_video_decode_end 永远不会被传入 NULL 缓冲区。
 */
